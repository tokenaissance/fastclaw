# FastAgent API Integration

Use this skill when integrating an upstream application with FastAgent as an
agent runtime.

The canonical reference is `docs/upstream-api.md` in the FastAgent repository.
...
Use `/api/*` or the `fastagent` CLI only for operator/admin workflows such as
...
Do not expose FastAgent API keys to browsers or mobile clients. Route calls
...
Authorization: Bearer <FASTAGENT_API_KEY>
Content-Type: application/json
X-Fastagent-Session-Key: <deterministic-session-key>
```
...
- `agent_id` selects the FastAgent agent. Body wins over
  `X-Fastagent-Agent-ID`.
- `user` is the upstream stable user ID. Body wins over
  `X-Fastagent-End-User`.
- `X-Fastagent-Session-Key` controls conversation history. Use a deterministic
...
Or skip provisioning and pass `user` on every chat call. FastAgent will lazy
...
1. Add server-side FastAgent client configuration:
   - base URL
   - API key
   - agent ID
2. Add or reuse upstream conversation IDs.
3. Map upstream user IDs to FastAgent `user` or `/v1/users`.
4. Implement streaming SSE parsing for `/v1/chat/completions`.
5. Persist or derive `X-Fastagent-Session-Key` per conversation.
...
- Do not call dashboard `/api/chat/stream` for upstream app chat unless you are
  embedding FastAgent's own dashboard semantics.
- Do not put FastAgent API keys in frontend code.
- Do not use email/display name as the stable FastAgent `user` value.
