...
func (a *Agent) handleSlashCommand(msg bus.InboundMessage) slashResult {
	...
	case "/version":
		return slashResult{handled: true, reply: fmt.Sprintf("⚡ FastAgent\nAgent: %s\nModel: %s", a.name, a.model)}
...
//
// Web / api: the chatter's UserID is the FastAgent user UUID — owner is
...
func (a *Agent) isAdminChatter(msg bus.InboundMessage) bool {
	// Web / api carry FastAgent UUIDs directly; owner check is sufficient.
	...
	if !ok || len(list) == 0 {
		...
		// ownership check: if the IM chatter's resolved FastAgent
...
func (a *Agent) slashStatus(msg bus.InboundMessage) slashResult {
	...
	status := fmt.Sprintf("⚡ FastAgent Status\n"+
		"─────────────────\n"+
		"Agent:       %s\n"+
		"Model:       %s\n"+
		"Personality: %s\n"+
