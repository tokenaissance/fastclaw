...
//   - /Users/<u>/... and /home/<u>/... that aren't fastagent-internal
...
func isExplicitHostScope(path string) bool {
	if isFastAgentInternalPath(path) || isSandboxOnlyPath(path) {
		return false
	}
...
// isFastAgentInternalPath reports whether path falls under FastAgent's
// runtime-managed dirs (~/.fastagent/...). These have dedicated routing
...
func isFastAgentInternalPath(path string) bool {
	if strings.HasPrefix(path, "~/.fastagent") {
		return true
	}
	if filepath.IsAbs(path) {
