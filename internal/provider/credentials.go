...
// user. Credentials live at ~/.fastagent/users/{userID}/credentials.json and
...
func legacyDeriveKey() []byte {
	...
	hash := sha256.Sum256([]byte("fastagent:" + hostname + ":" + home))
	return hash[:]
}
...
func deriveKeyForUser(userID, passphrase string) []byte {
	...
	var seed string
	if passphrase != "" {
		seed = "fastagent:user:" + userID + ":pp:" + passphrase
	} else {
		hostname, _ := os.Hostname()
		home, _ := os.UserHomeDir()
		seed = "fastagent:user:" + userID + ":host:" + hostname + ":" + home
	}
