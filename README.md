...
### Manage API keys from the CLI (`fastagent apikey …`)
...
fastagent apikey create --name "my-key" --type user [--owner <user-id>]

# List keys for a user (defaults to first super_admin)
fastagent apikey list [--owner <user-id>]

# Delete a key
fastagent apikey delete --id <apikey-id>

# Rotate a key (old token invalidated, new token shown once)
fastagent apikey rotate --id <apikey-id>
```
...
A `type=user` key combined with the `X-Fastagent-End-User` header enables
per-end-user data isolation without pre-registering users in FastAgent:

```
Authorization: Bearer <user-key-token>
X-Fastagent-End-User: <your-app-user-id>
```

FastAgent lazily mints a stable internal user for each unique
